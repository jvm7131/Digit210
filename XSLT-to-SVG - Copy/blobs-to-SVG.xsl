        ...
        <svg width="100%">
            ...
            <g transform="translate(20 500)">
                ...
                <xsl:for-each select="descendant::blob">
                    ...
                    <polygon
                        points="
                        {$x-spacer * position()}, {-50 - ($count * $y-spacer)}
                        {($x-spacer * position()) + ($count * $y-spacer)}, {-50 + ($count * $y-spacer)}
                        {($x-spacer * position()) - ($count * $y-spacer)}, {-50 + ($count * $y-spacer)}
                        "
                        fill="rgb(8, {$count*800}, {$count*60})"
                        stroke="black"
                        stroke-width="2"
                    />
            ...
            <g transform="translate(10 600)">
                ...
                <xsl:for-each select="descendant::blob">
                    ...
                    <polygon
                        points="
                        {$x-spacer * position()}, {-70 - ($count * $y-spacer)}
                        {($x-spacer * position()) + ($count * $y-spacer)}, {-70 + ($count * $y-spacer)}
                        {($x-spacer * position()) - ($count * $y-spacer)}, {-70 + ($count * $y-spacer)}
                        "
                        fill="rgb(128, {$count * 20}, {$count * 120})"
                        stroke="black"
                        stroke-width="2"
                        >
                        <animateTransform
